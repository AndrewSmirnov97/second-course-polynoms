...
Polinom::Polinom(string str){ //cстрока вида (знак)(коэфициенты даже единица)трибуквы со степенями в галочках
		string substr;
		substr.resize(30);
		h = NULL;
		int j = 1;
	...
	link* cur = h;
		...
		while (next->n != NULL){
			...
			if ((next->A.hide) == (cur->A.hide)){
				...
				coef1 = coef2 + coef1;
				cur->A.coef = coef1; //первому найденному звену зальем коэф и если отрицательный знак - то знак меняем
				exh = h;
				if (coef1 > 0){
